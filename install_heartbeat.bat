@echo off
REM ══════════════════════════════════════════════════════════════
REM  AetherKin - Heartbeat Installer
...
echo.
echo  =======================================
echo   AetherKin - Heartbeat Installer
echo  =======================================
echo.
...
schtasks /Delete /TN "AETHERKIN_DAWN" /F >nul 2>&1
schtasks /Delete /TN "AETHERKIN_ANCHOR" /F >nul 2>&1
schtasks /Delete /TN "AETHERKIN_HEARTBEAT" /F >nul 2>&1
echo       Done.

echo.
echo [2/4] Creating DAWN task (8:00 AM daily)...
schtasks /Create /TN "AETHERKIN_DAWN" /TR "\"%PYTHON%\" \"%SCRIPT%\" --dawn" /SC DAILY /ST 08:00 /F
if %ERRORLEVEL% neq 0 (
...
schtasks /Create /TN "AETHERKIN_ANCHOR" /TR "\"%PYTHON%\" \"%SCRIPT%\" --anchor" /SC DAILY /ST 20:00 /F
if %ERRORLEVEL% neq 0 (
...
schtasks /Delete /TN "AETHERKIN_HEARTBEAT_12" /F >nul 2>&1
schtasks /Delete /TN "AETHERKIN_HEARTBEAT_16" /F >nul 2>&1

schtasks /Create /TN "AETHERKIN_HEARTBEAT_12" /TR "\"%PYTHON%\" \"%SCRIPT%\" --heartbeat" /SC DAILY /ST 12:00 /F
if %ERRORLEVEL% neq 0 (
    echo [WARN] Failed to create HEARTBEAT 12pm task.
)

schtasks /Create /TN "AETHERKIN_HEARTBEAT_16" /TR "\"%PYTHON%\" \"%SCRIPT%\" --heartbeat" /SC DAILY /ST 16:00 /F
if %ERRORLEVEL% neq 0 (
...
echo    AETHERKIN_DAWN          8:00 AM  - Morning check-in
echo    AETHERKIN_HEARTBEAT_12 12:00 PM  - Midday pulse
echo    AETHERKIN_HEARTBEAT_16  4:00 PM  - Afternoon pulse
echo    AETHERKIN_ANCHOR        8:00 PM  - Evening reflection
echo.
...
echo    schtasks /Delete /TN "AETHERKIN_DAWN" /F
echo    schtasks /Delete /TN "AETHERKIN_ANCHOR" /F
echo    schtasks /Delete /TN "AETHERKIN_HEARTBEAT_12" /F
echo    schtasks /Delete /TN "AETHERKIN_HEARTBEAT_16" /F
echo.
pause
